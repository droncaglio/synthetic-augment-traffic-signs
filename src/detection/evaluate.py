...
def panorama_ground_truths(records_by_id: dict[str, dict], split_ids: list[str],
    ...
    """Subset-class GTs for the split panoramas, normalized to the panorama."""
    ...
    for img_id, pid in enumerate(split_ids):
        for o in records_by_id[pid]["objects"]:
            ...
            if cid is None:
                continue
            cx, cy, w, h = xyxy_to_yolo(tuple(o["xyxy"]), panorama_size, panorama_size)
...
def evaluate_split(records_by_id: dict[str, dict], split_ids: list[str], subset: dict,
    ...
    gts = panorama_ground_truths(records_by_id, split_ids, subset_ids, panorama_size)
    dets = panorama_detections(dets_by_panorama, split_ids)
    ap_result = compute_ap_by_size(dets, gts, class_names)
